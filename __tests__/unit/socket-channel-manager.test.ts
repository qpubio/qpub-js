...
describe('SocketChannelManager', () => {
    ...
    function createChannelManager(mocks: ReturnType<typeof createTestMocks>): SocketChannelManager {
    ...
    describe('Channel Creation and Management', () => {
        it('should create new channels on first access', () => {
            ...
            expect(mocks.logger.debug).toHaveBeenCalledWith('Creating new socket channel: test-channel');
        });
        ...
        it('should return existing channel for subsequent access', () => {
            ...
            expect(mocks.logger.debug).toHaveBeenCalledTimes(1); // Only called once for creation
        });
        ...
        it('should remove channels properly', () => {
            ...
            const channel = manager.get('test-channel');
            const removeListenersSpy = jest.spyOn(channel, 'removeAllListeners');
            ...
            expect(mocks.logger.debug).toHaveBeenCalledWith('Channel test-channel removed');
        });
    ...
    describe('Channel State Management', () => {
        ...
        it('should resubscribe all channels', async () => {
            ...
            const resubscribeSpy1 = jest.spyOn(channel1, 'resubscribe').mockResolvedValue();
            const resubscribeSpy2 = jest.spyOn(channel2, 'resubscribe').mockResolvedValue();

            await manager.resubscribeAllChannels();

            expect(resubscribeSpy1).toHaveBeenCalled();
            expect(resubscribeSpy2).toHaveBeenCalled();
            expect(mocks.logger.debug).toHaveBeenCalledWith('Resubscribing to 2 channels');
            expect(mocks.logger.debug).toHaveBeenCalledWith('Resubscribed to channel: channel-1');
            expect(mocks.logger.debug).toHaveBeenCalledWith('Resubscribed to channel: channel-2');
        });
    ...
    describe('Reset and Cleanup', () => {
        it('should reset all channels and clear the collection', () => {
            ...
            expect(mocks.logger.debug).toHaveBeenCalledWith('Resetting all socket channels');
        });
